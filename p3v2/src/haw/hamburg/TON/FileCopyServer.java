...
public class FileCopyServer {
		...
		while (true) {
			try {
				...
				if (connectionEstablished == false) {
					...
					recPacketCounter = 0;
					errorRate = 10000;
					System.out.println("New connection established with " + clientAdress.toString());
				}
				...
				if ((clientAdress.equals(receivedIPAddress)) && (clientPort == receivedPort)) {
					...
					fcReceivePacket = new FCpacket(udpReceivePacket.getData(), udpReceivePacket.getLength());

					long seqNum = fcReceivePacket.getSeqNum();
					recPacketCounter++;

					// Test on simulated error (packet checksum simulation)
					if ((errorRate > 0) && ((recPacketCounter % errorRate) == 0)) {
						...
						// Eval seq num
						if ((seqNum >= (rcvbase - windowSize)) && (seqNum < (rcvbase + windowSize))) {
							// ------ send ACK packet
				...
				System.out.println("Connection successfully closed, " + recPacketCounter + " packets received, file "
						+ destPath + " saved!\n");
	...
	private void sendAck(FCpacket fcRcvPacket) {
		...
		DatagramPacket udpAckPacket = new DatagramPacket(fcRcvPacket.getSeqNumBytes(), 8, clientAdress, clientPort);

		/* Anonymen Sende-Thread definieren und starten */
		new sendThread(udpAckPacket).start();
		testOut("ACK-Packet " + fcRcvPacket.getSeqNum() + " sent with delay " + DELAY);
	}
	...
	private boolean setParameters(FCpacket controlPacket) {
		...
		if (parameterArray.length == 3) {
			...
			try {
				...
				errorRate = Long.parseUnsignedLong(parameterArray[2]);
			} catch (NumberFormatException e) {
				System.err
						.println("Control Packet (seqNum 0): syntax error! No numeric parameter found: " + parameters);

				// Parameter wrong
				return false;
			}

			System.out.println("Server-Parameters set: " + destPath + " - WindowSize: " + windowSize + " - ErrorRate: "
					+ errorRate);

			// Parameter OK!
			return true;
		} else {
			System.err.println("Control Packet (seqNum 0) has wrong number of parameters: " + parameters);
